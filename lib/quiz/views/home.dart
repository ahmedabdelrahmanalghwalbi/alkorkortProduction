...
import 'package:korkort/quiz//views/quiz_play.dart';
import 'package:korkort/quiz/views/quiz_hint_1.dart';
...
class _HomeState extends State<Home> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        centerTitle: true,
        title:isArabic?Text("أمتحانات الأسئلة النظرية للفئة -B",style: TextStyle(
          fontFamily: "cairo",
          color: Colors.black
        ),):Text("Teori ifrågasätter tentor för klass B",style: TextStyle(
            fontFamily: "cairo",
            color: Colors.black
        ),),
        actions: [
    ...
    return GestureDetector(
      onTap: (){
        if(status=="passed"){
          ...
          Navigator.push(context, MaterialPageRoute(
//              builder: (context) => QuizPlay(widget.id)
          builder: (context)=>QuizHint1(id: widget.id,title: widget.title,description: widget.description,)
          ));
        }
      },
      child: Card(
